...
# NEO4J
Abbiamo inoltre implementato Neo4j all'interno del nostro programma con il fine di produrre dei grafi che mostrassero il tipo di relazione tra coppie di lettura e numeri nubeam. dalle foto(rappresentanti il micro sample presente in formato compresso su github) possiamo vedere come le coppie di lettura si distribuiscono all'interno del grafo. In particolare è interessante ooservare come le coppie non duplicate abbiano una struttura diversa rispetto alle coppie di lettura duplicate: le prime si dispongono in un grafo con 4 nodi e con relazioni 'uniche', ovvero non vi sono relazioni doppie tra un nodo e l'altro, mentre le coppie duplicate hanno più 'relazioni' con i corrispettivi nodi nubeam, questo indica che vi sono sequenze che condividono lo stesso numero nubeam e dunque sequenze che sono duplicate(prima foto).
